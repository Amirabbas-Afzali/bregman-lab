...
# ── W&B logging lives ONLY in this copy. stage_b_train.py is left byte-identical to the file
#    Taehyun's seed 0 ran, so the 8B seed replication can use it unchanged. Logging here is
#    write-only: it never touches the loss, the optimizer, or the RNG. ──────────────────────
_WB = None
...
def main():
    ...
    ap.add_argument("--wandb-project", default=None, help="W&B project; logging is OFF unless this is set")
    ap.add_argument("--wandb-entity", default=None, help="W&B entity/team (default: your default entity)")
    ap.add_argument("--wandb-name", default=None, help="run name (default: basename of --out)")
    args = ap.parse_args()
    ...
    global _WB
    if is_main and args.wandb_project:                 # rank 0 only: one run, not `world` runs
        import wandb
        _WB = wandb.init(project=args.wandb_project, entity=args.wandb_entity,
                         name=args.wandb_name or os.path.basename(args.out),
                         config=vars(args),
                         dir=os.path.dirname(args.out) or ".")   # keep ./wandb off /project
        print(f"[wandb] mode={os.environ.get('WANDB_MODE', 'online')} run={_WB.name} "
              f"url={getattr(_WB, 'url', 'n/a (offline)')}", flush=True)
    ...
    for step in range(1, args.steps + 1):
        ...
        if _WB is not None:      # per-step curve, for monitoring only: `hist` and the .json stay on
            _WB.log({"loss_step": float(np.mean(losses)),      # --log-every, so the saved artifacts are
                     "train_acc_step": float(np.mean(accs)),   # unchanged and comparable to seed 0.
                     "grad_norm_step": gnorm,
